...
public class Supermarket implements Selling {
    ...
    public boolean buy(Goods goods) {
        ...
        if (goods instanceof Apple) {
            int index = Logics.find(appleStorage, currentIndexApple, goods);
            if (index < 0) {
                return false;
            }
            Logics.remove(appleStorage, currentIndexApple, index);
            currentIndexApple--;
            wasRemoved = true;
        }
        if (goods instanceof Bread) {
            int index = Logics.find(breadStorage, currentIndexBread, goods);
            if (index < 0) {
                return false;
            }
            Logics.remove(breadStorage, currentIndexBread, index);
            currentIndexBread--;
            wasRemoved = true;
        }
        if (goods instanceof Milk) {
            int index = Logics.find(milkStorage, currentIndexMilk, goods);
            if (index < 0) {
                return false;
            }
            Logics.remove(milkStorage, currentIndexMilk, index);
            currentIndexMilk--;
            wasRemoved = true;
        }
        if (wasRemoved) {
